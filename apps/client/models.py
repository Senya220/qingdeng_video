...
class VideoStar(models.Model):
    video = models.ForeignKey(Video, related_name='video_star', on_delete=models.SET_NULL(), blank=True, null=False)
    name = models.CharField(max_length=100, null=False)
    # 演员身份[例如：主演]
...
class VideoStub(models.Model):
    video = models.ForeignKey(Video, related_name='video_stub', on_delete=models.SET_NULL(), blank=True, null=False)
